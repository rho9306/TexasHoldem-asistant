...
- [ ] 引擎内联（批次17 新增验证点）：**断网/飞行模式下打开过的页面选牌仍能正常计算**；弱网或被拦截的网络环境里选牌不再出现「计算引擎加载失败」；若出现「计算出错：×」属应用 bug，截图反馈开发者
...
- **「计算引擎加载失败」事件全记录（2026-09-23，批次16+17 解决，留档备查）**：
  - 症状演进：正常窗口选牌报「加载计算引擎失败」刷新无效 → 无痕窗口一度正常、后同样失败（先第一手好/下一轮后坏，后连第一手也坏）→ **换一台几乎没用过本站的新电脑同样报错**
  - 排查链（四步收敛）：①服务端实证——线上 index/入口/胶水/wasm 全 200+MIME 正确、Node 用线上产物真实实例化计算复验通过、Actions success → 排除产物层；②持久层排除法——刷新无效=SW缓存/localStorage（内存态问题刷新必好）、无痕正常一度指向 SW/扩展；③**多设备判别（决定性）**——新电脑无任何持久层仍报错 → 问题只跟网络路径走，不跟浏览器状态走；④请求差异分析——页面 URL（`/TexasHoldem-asistant/`、`index-*.js`）无敏感词能加载、引擎 URL（`poker_core.wasm/.js`）是**页面打开后按需发起的二次请求**，在用户网络环境（国内 github.io 间歇不通，疑似按 URL 关键词/连接特征拦截）下极易失败；同一时刻终端 curl 全 200 属不同请求路径，测不出用户侧拦截
  - 修复两批次：批次16（预热+报错文案拆分——只提前时机未消除请求，事后证明不充分，**但「说真话」的两句文案成为后续定位的关键工具**）→ 批次17（引擎内联进主包，引擎网络请求 2→0，根治：页面能打开引擎就一定在）
  - 提示假象解释：「下一轮后坏」实为切后台页面被系统静默回收重建后引擎需重新拉取恰逢拦截窗口，「点下一轮」只是时间上的巧合（扑克场景频繁切出去）；「无痕一度正常」是拦截规则的间歇性
  - 经验沉淀：①多设备复测是「环境问题 vs 产物/网络问题」的一刀切利器；②报错信息真实化是远程排障的前提；③对弱网用户，**任何「主包之外的后按需请求」都是故障面**，能内联的都应内联
...
## 十、第六轮：上线后迭代记录（2026-09-21~22，用户实测反馈驱动，批次 1-15 全部上线 ✅，测试基线 150）
...
- **UI 紧凑与大按钮（批次12）**：opponentCards `compact:!split`——手机对手标题行只留 牌桌/下一轮，**加/减对手统一走牌桌页调人数**（电脑保留＋/一键全员；手机失去批量设类型入口，用户明确取舍）；.stp 步进按钮 44×44+18px bold（原完全无样式）；.scenarios flex-wrap（44px 按钮后 ≤375px 溢出）
- **桌画像「无信号不判断」（批次13）**：批次7 的 TAG 兜底从胜率数学泄漏到展示层已纠正——tableProfile 排除「未标定且无观察（<20手）」对手；全无信号→null→UI「暂无画像」；部分覆盖→「n/m已标定」（basedOn 字段）；观察 VPIP（≥20手，**含 0**，`vpipObs != null` 非 `>0`）优先于类型预设；凶轴只有标注类型才有数据。**胜率数学（ranges 掩码）TAG 兜底不变**
...
- **批次8 迁移**：migrations 标记与对手列表两次写入之间若存储恰好失败会理论性重跑（极苛刻）；texas.migrations 单键扁平对象，**后续迁移须追加键勿整体覆盖**
- **批次10 行动建议**：c-bet 卡（纯纹理口径）与行动横幅（胜率口径）可能同屏口径并存；effStack=0（后手填 0）时建议显示怪异不崩溃；翻前平跟恒 1BB（不看 blind level，教学口径）
- **批次11 历史页/缓存**：空手数会话在任何筛选下不渲染→无删除入口（只能靠清空历史）；删除按钮 36px<44px；confirm 文案含用户输入为纯文本无 XSS；sw.js 自身更新可能延迟 ≤10 分钟
- **批次12 紧凑模式**：手机端无批量设类型入口（用户明确取舍，需要时可在牌桌页加回）；compact 下 onAdd/onPreset 为死参数
...
2. ⬜ 真机验收（§8.2 清单：功能/性能/兼容/离线/合规，含 F12 四视口；**新增验证点：结算弹窗流程、行动建议横幅、桌画像未标定提示、下一轮清空、手机紧凑标题行、引擎内联断网选牌（批次17）**）
3. ⬜ 用户后续新反馈（本轮节奏：反馈→批次实现+审查→推送部署→文档同步）
4. ⬜ 筹码体系候选方向（批次15 与用户讨论过、用户提出再排期）：**盲注体系**（设置页小盲/大盲，统一 BB 口径，治「1单位=1BB吗」的根源）、**筹码随行动自动推演**（跟注后自动扣后手入池，代价多一次确认）、**会话筹码曲线**（顶栏显示当前虚拟筹码，与 EV 双曲线呼应）
5. ⬜ 其它候选：庄位/轮次持久化（现为会话内存）、对手观察值一键修正、历史页导出表格化等
