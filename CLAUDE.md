...
**最后更新：** 2026-09-22（第六轮·批次 15 牌局结算完成，150 测试全绿；**用户已授权：以后每批次做完直接推送**，推送遇 github.com 间歇不通时退避重试即可）
**当前阶段：** 🎉 已上线：https://rho9306.github.io/TexasHoldem-asistant/ · 第六轮批次 1-15（核心闭环：选牌→建议→结算→筹码连续→复盘）。剩余待办：真机验收（§8.2，需用户执行）、用户后续新反馈迭代（§10.5）、筹码体系候选方向（§10.5.4）、各批次留档 Minor（§10.4）
...
- 测试基线：**150 vitest + 6 引擎基准全绿**；`npm run build` 通过；改 SW 后需手更 CACHE 版本号（`texas-web/public/sw.js`）；发布时同步手更设置页「关于」的批次号（settingsPage.js）
...
2. ⬜ 真机验收（§8.2 清单：功能/性能/兼容/离线/合规，含 F12 四视口；**新增验证点：结算弹窗流程、行动建议横幅、桌画像未标定提示、下一轮清空、手机紧凑标题行**）
